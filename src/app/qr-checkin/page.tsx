...
import { pilgrims as seedPilgrims, type Pilgrim } from '@/lib/mockData';
...
export default function QrCheckinPage() {
  const [attendance, setAttendance] = useState<Record<string, Attendance>>(
    Object.fromEntries(seedPilgrims.map((p) => [p.id, p.attendanceStatus]))
  );
  ...
  const filtered = seedPilgrims.filter(
  ...
  const handleScan = (e: React.FormEvent) => {
    ...
    const pilgrim = seedPilgrims.find((p) => p.id === id);
    if (!pilgrim) {
      toast.error(`No pilgrim found with ID "${scanInput}"`);
      return;
    }
    markPresent(pilgrim.id);
