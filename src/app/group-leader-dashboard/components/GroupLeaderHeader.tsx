...
export default function GroupLeaderHeader({ groupLeader: gl, allGroupLeaders, pilgrims }: GroupLeaderHeaderProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success(`${gl.groupId} removed.`);
      setDeleteOpen(false);
      const remaining = allGroupLeaders.filter((g) => g.groupId !== gl.groupId);
      if (remaining.length > 0) {
        router.push(`/group-leader-dashboard?group=${remaining[0].groupId}`);
      } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/20 backdrop-blur-sm fade-in">
          <div className="bg-card rounded-xl border border-border shadow-xl p-6 w-full max-w-sm mx-4 slide-up">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Removes {gl.name} ({gl.groupId}). If pilgrims are still assigned to this group, they need to be moved to another group first.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteOpen(false)} className="btn-secondary flex-1 justify-center">Cancel</button>
              <button onClick={handleDelete} className="flex-1 justify-center px-4 py-2 bg-[#DC2626] text-white rounded-lg text-sm font-medium hover:bg-[#B91C1C] transition-colors">
                Remove
              </button>
            </div>
          </div>
